...
###########################################################
# HUGE TODO: refactor sqlalchemy stuff to separate module #
###########################################################
...
class Orden(Base):
    ...
    def __repr__(self):
        return "<Orden(nro_orden='%s', clase_orden='%s', descripcion='%s', ubic_tecnica='%s', ubic_tecnica_desc='%s', geo_x='%s', geo_y='%s', fecha_creacion='%s', fecha_ini_extremo='%s', fecha_fin_extremo='%s', calle='%s', altura='%s', clave_modelo_txt='%s', tipo_obra_id='%s', status_operacion='%s', comuna='%s')>" % (self.nro_orden, self.clase_orden, self.descripcion, self.ubic_tecnica, self.ubic_tecnica_desc, self.geo_x, self.geo_y, self.fecha_creacion,
                                                                                                                                                                                                                                                                                                                               self.fecha_ini_extremo, self.fecha_fin_extremo, self.calle, self.altura, self.clave_modelo_txt, self.tipo_obra_id, self.status_operacion, self.comuna)
...
if not ev:
    session.add_all([TipoObra(name='Bacheo'), TipoObra(name='Senalizacion'), TipoObra(
        name='Tapas'), TipoObra(name='Materiales'), TipoObra(name='Vereda'), TipoObra(name='Cordon')])
    session.commit()
...
if not ev:
    ...
    with open('clave-tipo.csv', 'rb') as csvfile:
...
if not ev:
    # Si no existen, agregar a la tabla a partir del CSV
    with open('status_usuario-status.csv', 'rb') as csvfile:
        mappings = csv.reader(csvfile, delimiter=',', quotechar='"')
        for row in mappings:
            session.add(
